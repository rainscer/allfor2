@if((isset($small_cart_products) && count($small_cart_products)) || (Session::has('cart_id') && App::make('Cart')->checkPostedAndDeleted(Session::get('cart_id'))))
    <div class="basket" style="max-height: 877px;">
        <div class="basket-title">{{ trans('cart.title') }}</div>
        <div class="cart-products">
            ...
            @if(is_array($small_cart_products))
                @foreach($small_cart_products as $cart_product)
                    <div id="small-pr{{ $cart_product['product_id'] }}"
                         class="basket-product-item clearfix">
                        <div class="prod-img"><img
                                    src="{{ $cart_product['image'] }}"></div>
                        <div class="prod-desc">
                            {{ $cart_product[$local] }}
                            <div class="prod-count"><span
                                        class="quantity">{{ $cart_product['quantity'] }}</span>
                                x
                                $<span class="prod count-price">{{ $cart_product['price'] }}</span><span></span>
                            </div>
                            <div class="prod-count">
                                {{-- */ $delivery_type = isset($cart_product['delivery_type']) ? $cart_product['delivery_type'] : false /* --}}
                                {{ trans('cart.delivery') }}
                                : {{ \App\Models\CatalogProduct::staticCheckDeliveryType($delivery_type) }}
                            </div>
                        </div>
                        <div class="prod-delete">
                            <a href="{{ url('cart/delete') }}"
                               data-owner-id="{{ $cart_product['product_id'] }}"
                               class="ajaxActionDeleteProduct delete_prod_cart">
                            </a>
                        </div>
                    </div>
...
$now = \Carbon\Carbon::now(); /* --}}
        ...
        <div class="basket-total-count">
            {{ trans('cart.productsInCart') }}: <span
                    class="total-count">{{ $total_count }}</span>
        </div>
        <div class="basket-total-price">
            {{ trans('cart.total') }}: <span
                    class="price-basket">${{ $total_sum }}</span> {{--{{ $curency_code }}--}}
        </div>
        ...
        <div class="basket-buttons">
            <a href="{{ url('cart') }}"
               class="basket-order">{{ trans('cart.checkout') }}</a>
        </div>
        <div class="basket-buttons">
            <a href="{{  url('/cart/clean') }}"
               class="cart-clear">{{ trans('cart.clearCart') }}</a>
        </div>
    </div>
@else
    <div class="basket">
        <div class="empty_cart">{{ trans('cart.emptyCart') }}</div>
    </div>
@endif