    ...
    <div class="delivery_block {{ !isset($validated_address) ? '' : 'hidden' }}">
        ...
        <div>
            ...
            <div class="form-group width-2">
                <select class="delivery_city form-control chosen-select" autocomplete="off" id="d_user_city" name="d_user_city" required>
                    @if(Auth::check())
        ...
        @if(Auth::check())
            <div class="form-group">
                {!! Form::email('email',null, ['class'=>'form-control','id' => 'email', 'placeholder'=> trans('cart.yourEmail'), 'required'] ) !!}
            </div>
        @else
            <div class="form-group">
                {!! Form::email('email',null, ['class'=>'form-control','id' => 'email', 'placeholder'=> trans('cart.yourEmail'), 'required'] ) !!}
        ...
        <div class="pay-block centered">
            <div>
                <button type="submit" class="btn pay"><img src="{{ asset('/images/wayforpay_pay.png') }}" /></button>
            </div>
        </div>
        {!! Form::close() !!}
    </div>
