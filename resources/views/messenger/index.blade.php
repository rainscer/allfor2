    ...
    <div class="container-fluid user-setting-block all-threads">
                ...
                @if($threads->count() > 0)
                    @foreach($threads as $thread)
                        ...
                        <tr class="normal {!! $curRouteTrashed ? : $class !!}">
                            ...
                            {{--<p><small><strong>{{ trans('messenger.author') }}:</strong> {!! $thread->creator()->name !!}</small></p>--}}
                            <td class="width-19">
        ...
        @if ($threads instanceof \Illuminate\Pagination\AbstractPaginator && $threads->lastPage() > 1)
            ...
            <div class="goto-page">
                ...
                <a href="{{ route('messages') }}" class="goto-action">{{ trans('qa.goto') }}</a>
            </div>
            </div>
        @endif
        ...
        @if($support)
            <a class="btn message-submit" href="{{ url('chat') }}">{{ trans('user.chats') }}</a>
        @endif
    </div>
@endsection
