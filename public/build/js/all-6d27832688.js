...
$(document).ready(function () {
        ...
        if($.isEmptyObject(itemsData) == false) {
        	...
        	$.post(
                url_cart_update + url, {
                    ...
                    if (data.coupon == 0) $('.coupon_sum').html('<span style="color:red;">Невалидный купон</span>');
                    else $('.coupon_sum').html('');
                    if (data.coupon > 0) {
                        $('.coupon_sum').html('-$<span class="coupon">' + data.coupon + '</span>');
                        coupon_sum = data.coupon;
					}

					totalSumWithDelivery = (totalWeight * 0.015 - parseFloat(coupon_sum) + parseInt(totalSum)).toFixed(2);

                    total.text(totalSumWithDelivery);
                    totalWeightText.text(totalWeight);
                    totalDeliveryByWeight.text((totalWeight * 0.015).toFixed(2));
                    ...
                    $('.top-basket').empty().html(data.smallCart);
                    $('.cart_total').empty().html(data.cart_total_count);

                });
        }else{
            $('.main_title').text('There are no products in the cart, you will be redirected to the main page');
            $('.basket_w').text('');
    ...
    $(document).on('click', '.ajaxActionDeleteProduct', function (e) {
        ...
        var el = $(this),
            ...
            total = $('.total-price'),
            cart_page = $('.basket_container').length;
    ...
    $(document).on('submit', '.form-review', function (e) {
        ...
        $.post(el.attr("action"),
            ...
            .done(function (data) {
                ...
                $(el).find('input[type="email"]').val('').removeClass('borderRed');
    ...
    $(document).on('click', '.ajaxAddProductToCart', function (e) {
        ...
        $.post($(el).data('url'), {
            ...
            if(data.quest){
                ...
                $('.top-basket').empty().html( data.response_content );
                $('.cart_total').empty().html( data.cart_total_count );
            }
        });
    });
