    ...
    $(document).on('click', '.ajaxAddProductToCart', function (e) {
        ...
        $.post($(el).data('url'), {
            ...
            if(data.quest){
                ...
                $(message_block).find('.total-cart-in-pp').empty().text(data.cart_total_count);
                /*$(message_block).animateCss('fadeInDown');
                $(message_block).show();*/
                $(message_block).toggle("blind",200);
                $('.basket').empty().html( data.response_content );
                $('.cart_total_header').empty().html( data.cart_total );
            }
        });
    });
