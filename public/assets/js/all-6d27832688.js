...
$(document).ready(function () {
    ...
    function recalculation() {
        ...
        if($.isEmptyObject(itemsData) == false) {
            $.post(
                url_cart_update, {
                    ...
                    // totalSumWithDelivery = parseInt(data.packing_price * data.coef_delivery + data.ua_deliv_price * data.coef_delivery + parseInt(totalSum), 10);
							var costDelivery = Math.ceil(totalWeight / 100) * 2;
							$('.ua-deliv-price').text('$' + costDelivery);
							if (totalWeight % 100) $('.can-order').text(', так же Вы можете заказать еще товар весом до ' + (100 - totalWeight % 100) + 'г и сумма доставки не изменится');
							else $('.can-order').text('');
                    // get delivery cost as collection from delivery parameters
                    totalSumWithDelivery = parseInt(parseInt(data.ua_deliv_price) + parseInt(totalSum) + costDelivery);
            ...
            $('.basket-page-title').text('В корзине нет товаров, вы будете перенаправлены на главную страницу');
            $('.cart-body').text('');
